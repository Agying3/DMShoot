...
class Message:
    ...
    def from_douyin(cls, raw_msg: dict) -> "Message":
        ...
        server_id = raw_msg.get("server_message_id") or raw_msg.get("msg_id")
        conv_id = raw_msg.get("conversation_id", "")
        msg_index = raw_msg.get("index_in_conversation", 0)
        message_key = (
            f"douyin:{server_id}" if server_id
            else f"douyin:{conv_id}:{msg_index}" if conv_id and msg_index
            else ""
        )
        return cls(
            platform="douyin",
    ...
    def from_bilibili(cls, raw_msg: dict) -> "Message":
        ...
        talker_id = raw_msg.get("talker_id", "")
        ...
        return cls(
            ...
            raw=raw_msg,
            seq_id=seq_id,
            message_key=f"bilibili:{talker_id}:{seq_id}" if talker_id and seq_id else "",
        )
